...
class Timer {
  public:
    // Initialize the timer, to call the interrupt handler
    // "timerHandler" every time slice, the length of which is defined
    // by duration.
    Timer(VoidFunctionPtr timerHandler, size_t callArg, size_t duration, bool doRandom);
    ~Timer() {}
...
// Internal routines to the timer emulation -- DO NOT call these
  ...
  private:
    ...
    size_t period;		// time between interrupts
