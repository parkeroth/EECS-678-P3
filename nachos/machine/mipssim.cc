...
Machine::Run()
{
    ...
    DEBUG( (char *)DB_MACHINE , (char *)"Starting thread \"%s\" at time %d\n",
	       currentThread->GetName(), stats->totalTicks);


    DSTRM_EVENT_DATA(MACHINE, RUN, currentThread->Get_Id(), sizeof(int), &(stats->totalTicks), "print_int");
...
Machine::OneInstruction(Instruction *instr)
{
    ...
    instr->Decode(raw);
    if (DebugIsEnabled( (char *)DB_MACHINE ))
      {
	...
	break;
      ...
      case OP_LUI:
	DEBUG( (char *)DB_MACHINE , (char *)"Executing: LUI r%d,%d\n", instr->rt, instr->extra);
