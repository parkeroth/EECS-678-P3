...
Disk::Disk(char* name, VoidFunctionPtr callWhenDone, size_t callArg)
{
  ...
  DEBUG( DB_DISK , "Initializing the disk, 0x%x 0x%x\n", callWhenDone, callArg);
...
Disk::WriteRequest(int sectorNumber, char* data)
{
    ...
    DEBUG( DB_DISK , "Writing to sector %d\n", sectorNumber);
    Lseek(fileno, SectorSize * sectorNumber + MagicSize, 0);
...
Disk::ComputeLatency(int newSector, bool writing)
{
  ...
  if (!writing && seek == 0 &&
    ...
    {
      DEBUG( DB_DISK , "Request latency = %d\n", RotationTime);
...
Disk::UpdateLast(int newSector)
{
    ...
    DEBUG( DB_DISK , "Updating last sector = %d, %d\n", lastSector, bufferInit);
}
