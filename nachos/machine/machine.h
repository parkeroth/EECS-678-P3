...
#pragma interface "machine/machine.h"
...
#ifdef REMOTE_USER_PROGRAM_DEBUGGING
		     BreakpointException,   // Breakpoint encountered.
#endif
		     OutOfMemory,	    // We dont have enough memory.
		     
		     NumExceptionTypes
};

#ifdef REMOTE_USER_PROGRAM_DEBUGGING
#define BREAKPOINT(addr) machine->RaiseException(BreakpointException, addr);
#endif
...
// decode the binary representation of the instruction 
   void Decode(unsigned int binary_instr);
...
// Routines internal to the machine simulation -- DO NOT call these 
    ...
    // FIXME : addr should be unsigned int as well as value in WriteMem    
    bool ReadMem(int addr, int size, void* value);
...
//
// WARNING: the arguments to these macros are evaluated multiple times!

#ifdef HOST_IS_BIG_ENDIAN
#define WordToHost(word)						\
  ((uint32_t)(								\
	     (((uint32_t)(word) & (uint32_t)0x000000ffUL) << 24) |	\
	     (((uint32_t)(word) & (uint32_t)0x0000ff00UL) <<  8) |	\
	     (((uint32_t)(word) & (uint32_t)0x00ff0000UL) >>  8) |	\
	     (((uint32_t)(word) & (uint32_t)0xff000000UL) >> 24) ))
#define ShortToHost(sword)					\
  ((uint16_t)(							\
	     (((uint16_t)(sword) & (uint16_t)0x00ffU) << 8) |	\
	     (((uint16_t)(sword) & (uint16_t)0xff00U) >> 8) ));
#else
#define WordToHost(word)   word
#define ShortToHost(sword) sword
#endif /* HOST_IS_BIG_ENDIAN */

#define WordToMachine(word)   WordToHost  (word)
#define ShortToMachine(sword) ShortToHost (sword)
