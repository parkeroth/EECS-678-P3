...
output_file = "/dev/null" (type=xml)
local_modules = ["prp_filters.py"]
filters = [
	prp.prp()
]
"""
...
def genDSUIConfs():
	pp_config = open("prp.pp", "w")
	makefile = open("Makefile.prp", "w")
	makefile.write("all:\n")
	makefile.write("\trm -rf prp.pp.out\n")
	...
	for option in option_sets:
		...
		pp_config.write(pp_config_tmpl % (tag, dsui_bin_filename)) 
		makefile.write("\tdstream_pipeline -c prp.pp %s >> prp.pp.out&\n" % tag)
		pass
	pp_config.close()
	makefile.close()
	pass
