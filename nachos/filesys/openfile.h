...
class OpenFile {
  public:
    ...
    int ReadAt(void *into, size_t numBytes, int position) { 
    		Lseek(file, position, 0); 
		return ReadPartial(file, into, numBytes); 
		}	
    int WriteAt(void *from, size_t numBytes, int position) { 
    		Lseek(file, position, 0); 
		WriteFile(file, from, numBytes); 
		return numBytes;
		}	
    int Read(void *into, size_t numBytes) {
		int numRead = ReadAt(into, numBytes, currentOffset); 
		currentOffset += numRead;
		return numRead;
    		}
    int Write(void *from, size_t numBytes) {
...
class OpenFile {
  public:
    ...
    int Read(void *into, size_t numBytes); // Read/write bytes from the file,
