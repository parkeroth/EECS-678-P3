...
import cPickle
import matplotlib 
# Agg backend does not need an X connection to run, which is more
# convenient for ssh connections.
...
def create_histograms_from_pickled_file(filepath, outfile, title, 
    ...
    # to be used by the histogram.
        non_empty_interval_list = filter(lambda i: bool(i), all_intervals)

        if non_empty_interval_list:
            min_value = min( min(thread_intervals) 
                             for thread_intervals in non_empty_interval_list )
        else:
            min_value = 0
    else:
        min_value = rmin
    ...
    n, bins, patches = plt.hist( all_intervals,
                                 buckets, range=(min_value, max_value),
                                 histtype='bar')
...
if __name__ == '__main__':
    ...
    class Params_Set:
        ...
        def __init__(self):
            ...
            self.p.add_option('--min', action='store', type='int',
                              dest='range_min',
                              help='The MINIMUM range for a bucket in the histogram.'
                                   ' If this option is specified, any value below the '
                                   ' minimum range is placed within the underflow bucket.')

            self.p.add_option('--max', action='store', type='int',
                              dest='range_max',
                              help='The MAXIMUM range for a bucket in the histogram.'
                                   ' If this option is specified, any value below the '
                                   ' maximum range is placed within the overflow bucket.')

            self.p.add_option('--buckets', action='store', type='int',
                              dest='buckets',
                              help='The number of BUCKETS to use for the histogram.'
                                  ' Defaults to 10.')
        ...
        def parse(self):
            ...
            self.buckets         = self.options.buckets
            self.range_min       = self.options.range_min
            self.range_max       = self.options.range_max
